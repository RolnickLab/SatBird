# test dataloader (size of image, label, mask)
import os
import pandas as pd
import torch
import pytest
...
def test_data_loader(config, dataframe) -> None:
    ...
    dataset = SDMVisionMaskedDataset(
        df=dataframe[0:100],
        data_base_dir=config.data.files.base,
        env=config.data.env,
        env_var_sizes=config.data.env_var_sizes,
        num_species=config.data.total_species)

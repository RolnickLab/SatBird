...
def process_hotspot(
    ...
    return summer_targets_and_entries
...
def target_and_entry(
    ...
    targets = (
        df[["SCIENTIFIC NAME", "GLOBAL UNIQUE IDENTIFIER"]].groupby("SCIENTIFIC NAME").agg("count")
    )
...
def main():
    ...
    #summer_targets_dir = output_dir / "summer_targets"
    winter_targets_dir = output_dir / "winter_targets"

    #_remove_dir_if_it_exists(summer_targets_dir)#, second_phase_completed_file)
    #_remove_dir_if_it_exists(winter_targets_dir)#, second_phase_completed_file)

    #summer_targets_dir.mkdir(parents=True, exist_ok=False)
    winter_targets_dir.mkdir(parents=True, exist_ok=False)
    ...
    with mp.Pool(processes=n_processes) as pool:
            ...
            #     files_iterator,
            # ):
        for winter_data in map(functools.partial(process_hotspot, filler=filler),
                files_iterator,
         #       chunksize=worker_chunk_size,
            ): #pool.imap_unordered(
            winter_targets_and_entries = winter_data
            for season, target_dir, targets_and_entries in [
                    ("winter", winter_targets_dir, winter_targets_and_entries),
                ]:
                for hotspot_name, (target, entry_df) in targets_and_entries.items():
                    ...
                    if hotspot_name == "hotspotnotexist":
                        continue
                    logger.debug(
                            f"Writing target for season {season} and hotspot {hotspot_name}"
                        )
                    with open(target_dir / (hotspot_name + ".json"), "x") as f:
                        json.dump(target, f)

                        # Write the entry
                    out_file = output_dir / f"{season}_hotspots.csv"
                    entry_df.to_csv(
                            out_file,
                            index=False,
